...
impl Address {
		...
		for i in 0..2 {
			...
			if let Ok(a_apos) = SecretKey::from_slice(
				...
				(
					...
					arr_raa.serialize_vec(&secp, true).as_ref().to_vec(),
				)
					.hash()
					.as_bytes(),
			) {
				...
				if let Ok(q) = SecretKey::from_slice(
					...
					(
						format!("ecp{}", i).as_bytes().to_vec(),
						PublicKey::from_combination(&secp, vec![&aa_apos, bb])?
					...
					if let Ok(h) = SecretKey::from_slice(
						...
						(
							format!("ecp{}", i).as_bytes().to_vec(),
							aa_apos.serialize_vec(&secp, true).as_ref().to_vec(),
						)
							.hash()
							.as_bytes(),
					) {
	...
	pub fn get_view_tag_for_rx(
		...
		for i in 0..2 {
			let h = (
				format!("ecp{}", i).as_bytes().to_vec(),
				a_rr.serialize_vec(&secp, true).as_ref().to_vec(),
			)
				.hash();
			if let Ok(_) = SecretKey::from_slice(&secp, h.as_bytes()) {
				return Ok(h[0]);
			}
		}
		Err(Error::Secp(secp::Error::InvalidSecretKey))
	...
	pub fn get_view_tag_for_tx(
		...
		for i in 0..2 {
			let h = (
				format!("ecp{}", i).as_bytes().to_vec(),
				r_aa.serialize_vec(&secp, true).as_ref().to_vec(),
			)
				.hash();
			if let Ok(_) = SecretKey::from_slice(&secp, h.as_bytes()) {
				return Ok(h[0]);
	...
	pub fn to_vec(&self, secp: &Secp256k1) -> Vec<u8> {
		let mut wtr: Vec<u8> = Vec::with_capacity(65);
		match self.bech32_addr.inner_addr {
			InnerAddr::LiteStealthAddr { pubkey } => {
				let ser_p: Vec<u8> = pubkey.serialize_vec(&secp, true).as_ref().to_vec();
			...
			InnerAddr::StealthAddr {
				...
				let ser_v: Vec<u8> = v.serialize_vec(&secp, true).as_ref().to_vec();
				wtr.extend_from_slice(&ser_v[1..]);
				let ser_s: Vec<u8> = s.serialize_vec(&secp, true).as_ref().to_vec();
	...
	pub fn to_string(&self) -> String {
		...
		let hrp = match self.network {
			ChainTypes::Mainnet => "bmw",
			ChainTypes::Testnet => "tmw",
			ChainTypes::UserTesting => "mwu",
			ChainTypes::AutomatedTesting => "mma",
			ChainTypes::PerfTesting => "perf",
		};
...
impl FromStr for Address {
	...
	fn from_str(s: &str) -> Result<Address, Error> {
		...
		let network = match hrp.as_str() {
			"bmw" => ChainTypes::Mainnet,
			"tmw" => ChainTypes::Testnet,
			"mwu" => ChainTypes::UserTesting,
			"mma" => ChainTypes::AutomatedTesting,
			"perf" => ChainTypes::PerfTesting,
			...
			33 => {
				...
				Ok(Address {
					bech32_addr: Bech32Addr {
						version,
						inner_addr: InnerAddr::LiteStealthAddr {
							pubkey: PublicKey::from_slice(&secp, &data[0..33])?,
						},
					},
					network,
				})
			...
			65 => {
				...
				Ok(Address {
					bech32_addr: Bech32Addr {
						version,
						inner_addr: InnerAddr::StealthAddr {
							pubkey_view: PublicKey::from_slice(&secp, &data[0..33])?,
							pubkey_spend: PublicKey::from_slice(&secp, &data[33..])?,
						},
					},
					network,
				})
