...
pub enum Error {
	...
	TooLargeReadErr,
	/// When asked to write too much data
	TooLargeWriteErr,
...
impl From<std::num::TryFromIntError> for Error {
	fn from(e: std::num::TryFromIntError) -> Error {
		Error::TryInto(format!("{}", e))
	}
}

impl From<Utf8Error> for Error {
	fn from(e: Utf8Error) -> Error {
		Error::Utf8Error(format!("{}", e))
	}
}
...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match *self {
			...
			Error::TooLargeWriteErr => f.write_str("too large write"),
			Error::InvalidSwitchType => f.write_str("invalid switch type"),
			Error::TryInto(_) => f.write_str("try into failed error"),
			Error::Utf8Error(_) => f.write_str("utf8 error"),
		}
	}
}
...
impl error::Error for Error {
	...
	fn description(&self) -> &str {
		match *self {
			...
			Error::TooLargeWriteErr => "too large write",
			Error::InvalidSwitchType => "invalid switch type",
			Error::TryInto(_) => "try into",
